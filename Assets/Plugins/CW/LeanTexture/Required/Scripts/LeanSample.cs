	...
	public static class LeanSample
	{
		...
		public static NativeArray<float4> GetPixels(UnityEngine.Texture source, Allocator allocator, bool linear, ref int2 finalSize)
		{
			if (source != null)
			{
				...
				var desc          = new RenderTextureDescriptor(source.width, source.height, RenderTextureFormat.ARGBFloat, 0); desc.sRGB = false;
				var renderTexture = CwRenderTextureManager.GetTemporary(desc, "LeanSample GetPixels");
